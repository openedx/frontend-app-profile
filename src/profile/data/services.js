...
ensureConfig(['DISCOVERY_BASE_URL', 'LMS_BASE_URL'], 'Profile API service');
...
// GET Skills Quiz (Goal) data

function transformSkillsQuizData(username, data) {
  let transformedData = null;

  // there can be many entries per user, each will have a unique (autogenerated) id associated
  // with the data, so I assume the one with the largest id must be the most recent version of
  // the skill quiz
  try {
  const quizData = [];
  const { results } = data;
  results.forEach((quiz) => {
    const usernameMatch = quiz.username === username;
    if(usernameMatch){
      quizData.push(quiz);
    }
  });

  // get list of ids from the quizData for our user
  const ids = quizData.map(object => {
    return object.id;
  });
  // determine the largest id associated with the user (largest id == most recent)
  const maxId = Math.max(...ids);
  transformedData = quizData.find(x => x.id === maxId);
  } catch(error) {
    console.log(error);
  }
...
export async function getSkillsQuizData(username) {
  const url = `${getConfig().DISCOVERY_BASE_URL}/taxonomy/api/v1/skills-quiz/`;
  try {
    const { data } = await getHttpClient().get(url);
    return transformSkillsQuizData(username, data);
  } catch (e) {
    logError(e);
    return {};
  }
